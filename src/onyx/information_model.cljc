...
may be added by the user as the context is associated to throughout the task pipeline."
   ...
   {:summary "All options available to configure the virtual peers and development environment."
    ...
    :model {:onyx/id
            ...
            {:doc "Authentication method to use for authenticating with Google Cloud Storage (GCS) for checkpointing. The default :path, will use a file system path to [JSON key file](https://github.com/GoogleCloudPlatform/google-cloud-common/blob/master/authentication/readme.md#authentication). Note, if this method is used with a Docker Container, the JSON key file will need to be mounted as a volume in order for it to be discovered on the container file system. For added security, if the container is hosted in Kubernetes, it may be useful to mount the JSON key file as a [Kubernetes Secret] (https://kubernetes.io/docs/concepts/configuration/secret/). Use of :config will allow the following properties from a JSON key file to be provided in the peer-config (avoiding the need to mount a volume): `:onyx.peer/storage.gcs.auth.config.client-id`, `:onyx.peer/storage.gcs.auth.config.client-email`, `:onyx.peer/storage.gcs.auth.config.private-key`, `:onyx.peer/storage.gcs.auth.config.private-key-id`, `:onyx.peer/storage.gcs.auth.config.scopes`. It should be noted, that, if the :config option is chosen, it may risk exposure of a GCP Service Account's private key. This option is helpful for development but not recommended as the :path option is the most secure."
             :type :string
             :optional? true
             :default :path
             :added "0.11.0"}
            ...
            {:doc "When `:onyx.peer/storage.gcs.auth-type` is set to :path, this key must be set to enable onyx to find the location of the [JSON key file](https://github.com/GoogleCloudPlatform/google-cloud-common/blob/master/authentication/readme.md#authentication)."
             :type :string
             :required-when ["`:onyx.peer/storage.gcs.auth-type` is :path"]
             :optional? true
             :added "0.11.0"}

            :onyx.peer/storage.gcs.auth.config.client-id
            {:doc "When `:onyx.peer/storage.gcs.auth-type` is set to :config, this key must be set to enable authentication with Google Cloud Storage. This field is the 'client_id' field from the JSON key file (e.g. - generally a long integer value)."
             :type :string
             :required-when ["`:onyx.peer/storage.gcs.auth-type` is :config"]
             :optional? true
             :added "0.11.0"}

            :onyx.peer/storage.gcs.auth.config.client-email
            {:doc "When `:onyx.peer/storage.gcs.auth-type` is set to :config, this key must be set to enable authentication with Google Cloud Storage. This field is the 'client_emai' field from the JSON key file (e.g. - generally a GCP Service Account email generated by GCP like service-account-name@project-id.iam.gserviceaccount.com)."
             :type :string
             :required-when ["`:onyx.peer/storage.gcs.auth-type` is :config"]
             :optional? true
             :added "0.11.0"}
            ...
            {:doc "When `:onyx.peer/storage.gcs.auth-type` is set to :config, this key must be set to enable authentication with Google Cloud Storage. This field is the 'private_key' field from the JSON key file (e.g. - usually starts with -----BEGIN PRIVATE KEY-----\n)."
             :type :string
             :required-when ["`:onyx.peer/storage.gcs.auth-type` is :config"]
             :optional? true
             :added "0.11.0"}
            ...
            {:doc "When `:onyx.peer/storage.gcs.auth-type` is set to :config, this key must be set to enable authentication with Google Cloud Storage. This field is the 'private_key_id' field from the JSON key file (e.g. - hex based number)."
             :type :string
             :required-when ["`:onyx.peer/storage.gcs.auth-type` is :config"]
             :optional? true
             :added "0.11.0"}
            ...
            {:doc "When `:onyx.peer/storage.gcs.auth-type` is set to :config, this key must be set to allow the service account to make read or read/write claims on the bucket where it will be storing checkpoints. This field is defined in Google documentation for Cloud Storage with [possible values](https://cloud.google.com/storage/docs/authentication). If not supplied, will default to `[\"https://www.googleapis.con/auth/devstorage.read_write\"]`."
             :type [:string]
            ...
            {:doc "The bucket object name used to store and retrieve checkpoints from Google Cloud Storage."
             :type :string
             :default :none
             :optional? true
             :added "0.11.0"}
            ...
            {:doc "The Google Cloud [Storage Class](https://cloud.google.com/storage/docs/storage-classes) used for the `:onyx.peer/storage.gcs.bucket` provided. If this bucket has not been created yet, it will be created with the value provided. Note, if using Google Container Engine, GKE, the REGIONAL option is the best supported choice."
             :type :string
             :choices ["REGIONAL" "MULTI_REGIONAL" "NEARLINE" "COLDLINE ""STANDARD"]
             :default :none
             :optional? true
             :added "0.11.0"}
